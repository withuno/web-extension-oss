...
export const ViewSaveAddressHint = UnoOrchestrator.registerAction({
  id: "product-hints/view-save-address-hint",
  ...
  async execute(input: LayerPosition.Floating): Promise<void> {
    ...
    if (!seenProductHints.saveAddress) {
      await orchestrator.useAction(CreateAnalyticsEvent)({
        type: "Product hint viewed: save address",
      });

      await orchestrator.useAction(CreateLayer)({
        path: Path.SaveAddressHint,
        position: input,
      });
