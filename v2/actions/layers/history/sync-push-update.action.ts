...
export const SyncPushUpdate = UnoOrchestrator.registerAction({
  id: "layer/history/sync-push",
  zone: Zone.Content,
  async execute(input: { layer: Layer; update: History.State }): Promise<void> {
    ...
    if (currentHistory) {
      const nextEntries = [...currentHistory.entries.slice(0, currentHistory.index + 1), input.update.location];
