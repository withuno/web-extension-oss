...
export function templatize(node: Element | null): string | undefined {
  if (!node) {
    return undefined;
  }
  ...
  const tree = findPotentialTemplates(node, possibleTemplates);
  if (!tree) {
...
function findPotentialTemplates(node: Node, possibleTemplates: PossibleTemplates): JsonNode | null {
  if (node.nodeType === Node.ELEMENT_NODE) {
    ...
    const templateHash = `${element.tagName}#${Object.keys(attributes).sort()}#${children
      .map((c) => c.templateHash)
      .join("|")}`;

    const templateValues = Object.values(attributes).concat(children.flatMap((c) => c.templateValues));
...
function optimizeTemplate(template: PossibleTemplate): OptimizedTemplate {
...
function chooseTemplates(templates: Record<string, OptimizedTemplate>): Record<string, OptimizedTemplate> {
  ...
  for (const template of Object.values(templates).sort((t) => -t.depth)) {
    ...
    if (template.nodes.length - (consumedTemplateCounts[template.hash] ?? 0) < 3 || template.depth < 3) {
      continue;
    }
